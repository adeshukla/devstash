...
- Husky + lint-staged (Prettier via lint-staged; ESLint runs separately via `pnpm lint` / CI)
- .gitattributes for CRLF normalization
- ESLint: wired up (2026-07) via flat config (`eslint.config.mjs`, `eslint-config-next/core-web-vitals`). `pnpm lint` runs for real and is in CI (`typecheck` job)
...
| Editing `globals.css` → dev server serves STALE CSS                        | webpack dev CSS cache doesn't reliably pick up `globals.css` edits (survives hard-reload AND a fresh `next dev`). Verify via `document.styleSheets`; if stale, `rm -rf .next` and restart. Prod `pnpm build` is fine                                                                                 |
| Mobile perf: continuous `background-position` gradient anims               | `.text-gradient-animate` / `.gradient-border-animated` animate a NON-composited paint property → repaint every frame on the main thread (LCP element never settles; TBT stays low so it hides). Static ≤767px; animate only in `@media (min-width:768px) and (prefers-reduced-motion:no-preference)` |
...
| `FlatCompat` + `eslint-config-next` legacy bridge crashes on ESLint 9      | `compat.extends('next/core-web-vitals', ...)` throws `Converting circular structure to JSON`. Fix: import the flat config directly — `import nextConfig from 'eslint-config-next/core-web-vitals'` — no `FlatCompat`/`@eslint/eslintrc` needed                                                       |
| `react-hooks/set-state-in-effect` (new strict rule) flags legit mount-sync | Theme resolution, reduced-motion checks, route-change close, one-shot flags are safe single `setState` calls, not cascading loops. Downgraded to `'warn'` in `eslint.config.mjs` rather than restructuring working components                                                                        |
...
husky ^9.1.7
lint-staged ^17.0.5
@eslint/eslintrc ^3.3.5        # installed but unused — do NOT use FlatCompat, it crashes on ESLint 9 (see Known Issues)
...
# ESLint stack — wired up and active via eslint.config.mjs (2026-07)
eslint ^9
