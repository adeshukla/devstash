...
The cost is real too: publishing requires a commit and a deploy, and there's no "edit from my phone." For a site where I'm the only author, that trade was worth it. Owning every role on a project like this is its own subject, and I wrote up [what AI assistance actually removes from that, and what it doesn't](/blog/one-person-product-team-vibe-coding).
...
That mostly-server-rendered shape is half of why this site [isn't on React Compiler yet](/blog/why-devstash-isnt-on-react-compiler-yet); the other half is exact version pins.
