...
3. **`llms.txt` validity** — a plain-text file at the site root describing the site's purpose and structure for AI systems. [Search Engine Land's coverage of the check](https://searchengineland.com/google-llms-txt-chrome-lighthouse-478246) confirms it specifically verifies the file contains real Markdown-formatted links (`[label](url)`), not bare URLs — bare URLs are harder for a language model to parse confidently as "this text names that destination."
