...
That default, together with file-based routing, is also what makes an App Router codebase easy for an AI coding agent to navigate correctly. I wrote that up in [Next.js 16 App Router in the Age of Agent-Driven Development](/blog/nextjs-app-router-agent-driven-dev).
...
If this is the error that brought you here, I've written it up properly — why the build fails when dev didn't, where exactly the boundary has to sit, and the two "fixes" that quietly cost you performance — in [Fixing 'useSearchParams() should be wrapped in a suspense boundary'](/blog/usesearchparams-suspense-boundary-build-error).
