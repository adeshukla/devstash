...
On a project using Tailwind, this step maps directly onto editing the token layer — for DevStash that's the `@theme` block in `globals.css`, since [Tailwind v4 moved tokens out of `tailwind.config.ts` entirely](/blog/tailwind-v4-css-first-config-migration). Having the tokens defined before writing the first component means I'm never eyeballing a "close enough" spacing value mid-build.
